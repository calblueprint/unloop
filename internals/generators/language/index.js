...
function languageIsSupported(language) {
  try {
    fs.accessSync(`app/javascript/translations/${language}.json`, fs.F_OK);
    return true;
  } catch (e) {
    return false;
  }
...
module.exports = {
  ...
  actions: ({ test }) => {
    ...
    actions.push({
      type: 'add',
      path: '../../app/javascript/translations/{{language}}.json',
    ...
    if (!test) {
      actions.push(() => {
        const cmd = 'yarn run extract-intl';
        exec(cmd, (err, result) => {
          if (err) throw err;
